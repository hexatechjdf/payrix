...
@push('script')
        ...
        $(document).ready(function() {
                ...
                `);
                        ...
                        if (office.selected_location_id) {
                            row.find('.action-cell').html(
                                '<button class="btn btn-success btn-sm fetch_initial">Fetch Initial</button>'
                            );
                            let loc = locations.find(l => l.id == office.selected_location_id);

                            row.find('.destination-field')
                                .attr('data-location-id', office.selected_location_id);
            ...
            $('#saveMapping').on('click', function() {
                ...
                $('#officesTable tbody tr').each(function() {
                    ...
                    let locationId = $(this).find('.destination-field').data('location-id');
                ...
                $.post("{{ route('admin.mappings.offices.store.data') }}", {
                        _token: "{{ csrf_token() }}",
                        mappings: mappings
                    })
                    .done(function(res) {
    ...
    <script>
        ...
        $(document).on('click', '.fetch_initial', function() {
            ...
            Swal.fire({
                title: 'Confirmation',
                text: 'Do you want to fetch the data?',
                icon: 'question',

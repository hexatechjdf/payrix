...
@push('script')
    ...
    <script>
        $(document).ready(function() {
            ...
            $.ajax({
                url: '{{ route('mappings.employees.fetch.data') }}',
                type: 'GET',
                dataType: 'json',
            ...
            `);
                        ...
                        // Existing mapping
                        if (employee.selected_user_id) {
                            let user = users.find(u => u.id == employee.selected_user_id);

                            row.find('.destination-field')
                                .attr('data-user-id', employee.selected_user_id);

                            if (user) {
                                row.find('.destination-field').val(user.name);
                            }

                        } else {
                            // Auto match by email
                            let matchUser = users.find(
                                u => u.email && u.email === employee.employee_email
                            );

                            if (matchUser) {
                                row.find('.destination-field')
                                    .val(matchUser.name)
                                    .attr('data-user-id', matchUser.id);
                            }
                        }
        ...
        `);
            ...
            $('#saveMapping').on('click', function() {
                ...
                $('#officesTable tbody tr').each(function() {
                    ...
                    let userId = $(this).find('.destination-field').data('user-id');
                ...
                if (!mappings.length) {
                    Swal.fire({
                        ...
                        text: 'Please select at least one employee-user mapping.'
                    });
                ...
                $.post("{{ route('mappings.employees.store.data') }}", {
                        _token: "{{ csrf_token() }}",
                        mappings: mappings
                    })
                    .done(function(res) {
